...
pub fn rotations(entity:u64,game_process: &GameProcess) -> [f32; 2] {
...
pub fn get_entity_position(entity:u64,game_process: &GameProcess) -> Position
{
    let entity_klass = game_process.find_class("com/craftrise/m9");
    ...
    fn tick(&mut self, game: &GameProcess) {
        ...
        if self.last_target.time_since() < 5000 && self.last_target.target != 0 {
            ...
            if distance > 1.0 && distance < 4.7 {
                ...
                if (current_rotations[0] - rotations[0]).abs() > self.speed as f32 + 1.0 || mouse_over_entity != self.last_target.target {
